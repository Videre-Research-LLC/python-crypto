...
'''
Encrypt and decrypt data using RSA certificate
...
encoded_key_filename = "rsa_key.bin"
private_key_filename = "rsa_private.pem"
public_key_filename = "rsa_public.pem"
config_file_name = '.rsa_values.conf'
...
def extract_keys(args):
    ...
    print('Created private key file %s (File can decrypt data and is not password-protected, keep it safe!)' % private_key_file)
    print('Created public key file %s (distribute this one to anyone who needs to encrypt data, it cannot be used for decryption!)' % public_key_file)
...
def create_keys(args):
    ...
    file_out = open(encoded_key_file, "wb")
    file_out.write(encrypted_key)

    os.chmod(encoded_key_file, 0o400)
    # os.chmod(private_key_filename, 0o400)
    print('Created password-protected private/public keys file %s' % encoded_key_file)
...
def main():
    ...
    get_apps_p.add_argument('-v', '--value', action='store',
                            required=False, help='The name of the file to encrypt')
    ...
    try:
        if args.key:
            ...
            global private_key_file
            global encoded_key_file
            public_key_filename = args.key + '_public.pem'
            private_key_filename = args.key + '_private.pem'
            encoded_key_filename = args.key + '_key.bin'
            private_key_file = os.path.expanduser(os.path.join('~/', private_key_filename))
            public_key_file = os.path.expanduser(os.path.join('~/', public_key_filename))
            encoded_key_file = os.path.expanduser(os.path.join('~/', encoded_key_filename))
